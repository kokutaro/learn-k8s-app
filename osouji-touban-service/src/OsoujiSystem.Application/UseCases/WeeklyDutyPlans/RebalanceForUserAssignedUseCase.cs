...
public sealed record RebalanceForUserAssignedRequest : IRequest<ApplicationResult<DomainUnit>>
{
    public required WeeklyDutyPlanId PlanId { get; init; }
    public required UserId AddedUserId { get; init; }
}

public sealed class RebalanceForUserAssignedUseCase
    : IRequestHandler<RebalanceForUserAssignedRequest, ApplicationResult<DomainUnit>>
{
    ...
    public RebalanceForUserAssignedUseCase(
    ...
    public Task<ApplicationResult<DomainUnit>> Handle(RebalanceForUserAssignedRequest request, CancellationToken ct)
    {
        return UseCaseExecution.InTransaction(
            ...
            async token =>
            {
                ...
                var engineResult = await _planComputationService.RebalanceForUserAssignedAsync(area, plan, request.AddedUserId, token);
                if (engineResult.IsFailure)
                {
                    return ApplicationResult<DomainUnit>.FromDomainError(engineResult.Error);
                }

                var rebalanceResult = plan.RebalanceForUserAssigned(
                    request.AddedUserId,
