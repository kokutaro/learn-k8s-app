...
internal sealed class CachedCleaningAreaReadRepository(
    ...
    IOptions<InfrastructureOptions> options) : ICleaningAreaReadRepository
{
    ...
    private static string BuildQuerySignature(ListCleaningAreasQuery query)
        => $"userId={query.UserId?.ToString("D") ?? string.Empty}|cursor={query.Cursor ?? string.Empty}|limit={query.Limit}|sort={(int)query.Sort}";
